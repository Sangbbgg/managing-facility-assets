...
from app.models.record import SealRecord
from app.models.asset import Asset
from ._common import write_header, style_data_row

HEADERS = ["자산코드", "설비명", "날짜", "봉인번호", "행위", "처리자", "비고"]
WIDTHS  = [18, 20, 12, 14, 10, 12, 25]
...
async def build(date_from: date, date_to: date, db: AsyncSession) -> str:
    stmt = (
        ...
        .where(
            SealRecord.record_date >= date_from,
            SealRecord.record_date <= date_to,
        )
        .order_by(SealRecord.record_date)
    )
    ...
    ws_sum.cell(2, 1, f"총 봉인지 처리 건수: {len(rows)}")
    ...
    for month_key, items in sorted(monthly.items()):
        ...
        for i, (rec, asset) in enumerate(items, 2):
            ...
            ws.cell(i, 4, rec.seal_number or "")
            ws.cell(i, 5, rec.action)
            ws.cell(i, 6, rec.handler or "")
            ws.cell(i, 7, rec.notes or "")
